...
def create_tokens_list():
    ...
    for item in documentFiles:
        ...
        temp_tokens = [token.lower()
                       for token in temp_tokens if token.isalnum()]
        temp_tokens = [token for token in temp_tokens if token not in nltk.corpus.stopwords.words(
            'english')]  # Removing stop words
        temp_tokens = [stemmer.stem(token)
                       for token in temp_tokens]  # Stemming
